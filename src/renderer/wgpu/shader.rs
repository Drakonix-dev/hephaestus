...
use crate::{assets::{BindGroupLayout, BindingDesc, BindingType, MaterialUniforms, ShaderDefinition, ShaderDevice, ShaderHandle, ShaderStage}, builtin::BuiltinShader, renderer::wgpu::Surface};

pub(crate) struct ShaderManager<'a> {
...
impl<'a> ShaderManager<'a> {
    pub(crate) fn new(surface: &'a Surface<'a>) -> Self {
        ...
        manager.register_builtin_shaders();
    ...
    fn register_builtin_shaders(&mut self) {
        ...
        let handle = self.create_shader(&simple_color);
        self.builtin.insert(BuiltinShader::SimpleColor, handle);
    }
}

impl<'a> ShaderDevice for ShaderManager<'a> {
    ...
    fn create_shader(&mut self, def: &ShaderDefinition) -> ShaderHandle{
        ...
        let shader = self.surface.device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        self.shaders.insert(handle, ShaderInstance {
            handle,
            layout: def.layout.to_wgpu(&self.surface.device),
            module: shader,
        });
